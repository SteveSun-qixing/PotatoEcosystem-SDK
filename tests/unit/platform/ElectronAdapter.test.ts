...
describe('ElectronAdapter', () => {
  ...
  let testDir: string;
  let testPath: string;
  const testData = new TextEncoder().encode('Hello, Electron!').buffer;

  beforeEach(async () => {
    adapter = new ElectronAdapter();
    
    // 创建临时测试目录
    testDir = path.join(os.tmpdir(), `chips-electron-test-${Date.now()}`);
  ...
  describe('基础属性', () => {
    it('应该返回正确的平台类型', () => {
      expect(adapter.platform).toBe(Platform.Electron);
    });
  ...
  describe('Electron特有功能', () => {
    ...
    it('showInFolder应该不抛出错误', async () => {
      // 在非Electron环境中，应该静默失败
      await expect(adapter.showInFolder(testPath)).resolves.not.toThrow();
    });

    it('openExternal应该不抛出错误', async () => {
      // 在非Electron环境中，应该静默失败
      await expect(adapter.openExternal(testPath)).resolves.not.toThrow();
    });
  });
  ...
  describe('与Node适配器的兼容性', () => {
    it('应该支持所有Node适配器的基本功能', async () => {
      // 这个测试确保Electron适配器能够完全替代Node适配器
      ...
      expect(data.byteLength).toBe(testData.byteLength);
